      ...
      {data.map((d, i) => (
        <div key={i} className="flex items-center gap-3">
          <div
            ...
            style={{ background: i < 3 ? "#E6B422" : "#EFEBE9", color: i < 3 ? "#3E2723" : "#8D6E63" }}
          >
            {i + 1}
          </div>
          <div className="flex-1 min-w-0">
            <div className="text-[13px] font-semibold mb-1 truncate" style={{ color: "#3E2723" }}>{d.rule}</div>
...
export default function DashboardComboAnalysis() {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([
      getAssociationRules(0.03, 0.25).catch(() => ({ rules: [], total_transactions: 0 })),
  ...
  if (loading) return <Loading label="Đang phân tích dữ liệu nguyên liệu..." />;

  const chartData = (rules?.rules || []).slice(0, 5).map((r) => ({
    rule: `${r.antecedent} -> ${r.consequent}`,
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-display text-2xl font-bold" style={{ color: "#3E2723" }}>Phân tích quan hệ nguyên liệu</h1>
        <p className="text-sm" style={{ color: "#8D6E63" }}>
          Phân tích từ {rules?.total_transactions || 0} sản phẩm thật của Open Food Facts, mỗi sản phẩm là một giao dịch nguyên liệu.
        </p>
      </div>

      <div className="bg-white rounded-2xl p-6" style={{ border: "1px solid #E8DDD4" }}>
        <h3 className="font-display text-lg font-bold mb-1" style={{ color: "#3E2723" }}>Top cặp nguyên liệu phổ biến</h3>
        <p className="text-[12px] mb-5" style={{ color: "#8D6E63" }}>
          Tỷ lệ sản phẩm chứa nguyên liệu B khi đã có nguyên liệu A.
        </p>
        <BarChart data={chartData} />
      </div>

      <div className="bg-white rounded-2xl p-6" style={{ border: "1px solid #E8DDD4" }}>
        <h3 className="font-display text-lg font-bold mb-4" style={{ color: "#3E2723" }}>Chi tiết luật kết hợp nguyên liệu</h3>
        {tableData.length > 0 ? (
          <div className="overflow-x-auto">
            <table className="w-full text-sm" style={{ borderCollapse: "separate", borderSpacing: 0 }}>
              <thead>
                <tr>
                  {["#", "Nếu có", "", "Thường có thêm", "Confidence", "Lift", "Số lần"].map((h) => (
                    <th
                      key={h}
                      className="text-left text-[11px] font-bold uppercase tracking-wider px-3 py-2.5"
                      style={{ color: "#4E342E", background: "#FDF6EC", borderBottom: "2px solid #E8DDD4" }}
                    >
                      {h}
                    </th>
                  ))}
          ...
          <p className="text-center py-8 text-sm" style={{ color: "#8D6E63" }}>Chưa đủ dữ liệu để phân tích nguyên liệu.</p>
        )}
      </div>

      <div className="bg-white rounded-2xl p-6" style={{ border: "1px solid #E8DDD4" }}>
        <h3 className="font-display text-lg font-bold mb-3" style={{ color: "#3E2723" }}>Mức bao phủ luật kết hợp</h3>
        <div className="flex items-center gap-6">
          <div className="relative w-24 h-24 flex-shrink-0">
            <svg viewBox="0 0 100 100" className="w-full h-full -rotate-90">
              ...
              <circle
                ...
                stroke="#16A34A"
                strokeWidth="8"
                strokeDasharray={`${(recRate?.rate || 0) * 2.64} 264`}
                strokeLinecap="round"
              />
            </svg>
          ...
          <div>
            <p className="text-sm font-semibold" style={{ color: "#3E2723" }}>
              {recRate?.orders_with_recs || 0} / {recRate?.total_orders || 0} sản phẩm có cặp nguyên liệu phổ biến.
            </p>
            <p className="text-[12px] mt-1" style={{ color: "#8D6E63" }}>
              Chỉ số này dùng để chứng minh luật Apriori được khai phá từ dữ liệu nguyên liệu thật, không dựa vào đơn hàng mô phỏng.
            </p>
          </div>
        </div>
      </div>
