...
export default function DashboardOverview() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      getAssociationRules(0.03, 0.15).catch(() => ({ rules: [] })),
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl p-6" style={{ border: "1px solid #E8DDD4" }}>
        ...
        <p className="text-[12px] mb-5" style={{ color: "#8D6E63" }}>
          Biểu đồ thể hiện tỷ lệ phần trăm khách hàng gọi thêm món B khi đã gọi món A (dựa trên {rules?.total_transactions || 0} đơn hàng)
        </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-2xl p-6" style={{ border: "1px solid #E8DDD4" }}>
          ...
          <div className="flex items-center gap-6">
            ...
            <div>
              <p className="text-sm font-semibold mb-1" style={{ color: "#3E2723" }}>{recRate?.orders_with_recs || 0} / {recRate?.total_orders || 0} đơn</p>
              <p className="text-[12px] leading-relaxed" style={{ color: "#8D6E63" }}>
                Tỷ lệ đơn hàng có chứa nhóm món complementary — chứng minh hệ thống gợi ý AI đang hoạt động hiệu quả.
              </p>
            </div>
          </div>
        </div>
