/**
 * [V6] DishCard — Thêm nút "Quick Add" ngay trên thẻ
 * Người dùng bấm nút + là món tự động thêm vào giỏ, không cần vào trang chi tiết
 * Sử dụng formatCurrency chuẩn "100.000 ₫"
 */
...
export default function DishCard({ dish, badge }) {
  ...
  return (
    <Link to={`/dish/${id}`}
      ...
      onMouseEnter={(e) => { e.currentTarget.style.boxShadow = "0 12px 40px rgba(62,39,35,0.12)"; e.currentTarget.style.borderColor = "#F5D680"; }}
      onMouseLeave={(e) => { e.currentTarget.style.boxShadow = "none"; e.currentTarget.style.borderColor = "#E8DDD4"; }}>
      ...
      <div className="aspect-[16/10] flex items-center justify-center relative overflow-hidden"
        ...
        {/* [V6] Price badge — formatCurrency chuẩn */}
        <div className="absolute top-3 right-3 text-[12px] font-bold px-3 py-1.5 rounded-full shadow-md"
             style={{ background: "linear-gradient(135deg, #E6B422, #D4A017)", color: "#3E2723", boxShadow: "0 2px 8px rgba(230,180,34,0.35)" }}>
          {formatCurrency(price)}
        </div>
        {badge && (
          <div className="absolute top-3 left-3 text-[11px] font-bold px-3 py-1.5 rounded-full text-white flex items-center gap-1"
               style={{ background: "rgba(62,39,35,0.85)", backdropFilter: "blur(8px)" }}>{badge}</div>
        )}
      </div>
      ...
      <div className="p-5">
        ...
        <div className="flex items-center justify-between mb-3">
          {dish.avg_rating && (
            <div className="flex items-center gap-1 text-[13px] font-semibold px-2.5 py-1 rounded-lg" style={{ background: "#FDF3D7", color: "#D4A017" }}>
              ⭐ {typeof dish.avg_rating === "number" ? dish.avg_rating.toFixed(1) : dish.avg_rating}
            </div>
          )}
