...
def apriori_ing_suggestions(ingredients: str):
    """Endpoint cũ được giữ để tránh lỗi client; đề tài mới không khai phá quan hệ nguyên liệu."""
    ing_list = [i.strip() for i in ingredients.split(",") if i.strip()]
    return {
        "ingredients": ing_list,
        "suggestions": [],
        "message": "Đề tài đã chuyển sang khai phá mối quan hệ giữa các món ăn trong set menu, không phân tích nguyên liệu.",
    }
