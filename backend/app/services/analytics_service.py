...
class AnalyticsService:
    ...
    # ===== 1. ASSOCIATION RULES (Python thuần) =====
    def get_association_rules(self, min_support=0.03, min_confidence=0.2, dish_type=None) -> Dict:
        orders = self.db.query(Order).all()
        transactions = []
        for o in orders:
            try:
                items = [int(x.strip()) for x in o.dish_ids.split(",") if x.strip()]
                if len(items) >= 2:
        ...
        n = len(transactions)
        if n < 3:
            return {"rules": [], "frequent_itemsets": [], "total_transactions": n,
                    "min_support": min_support, "min_confidence": min_confidence}

        # Dish lookup
        all_dishes = {d.id: d for d in self.db.query(Dish).all()}
        names = {did: d.name for did, d in all_dishes.items()}
        ...
        freq1 = {i for i, c in item_cnt.items() if c / n >= min_support}

        # Pair counts
        pair_cnt = defaultdict(int)
        for t in transactions:
            items = sorted(t & freq1)
            for i in range(len(items)):
                for j in range(i+1, len(items)):
                    pair_cnt[(items[i], items[j])] += 1

        freq2 = {p: c for p, c in pair_cnt.items() if c / n >= min_support}

        # Build rules
        rules = []
        for (a, b), c in sorted(freq2.items(), key=lambda x: -x[1])[:25]:
            sup = c / n
            conf_ab = c / item_cnt[a] if item_cnt[a] else 0
            conf_ba = c / item_cnt[b] if item_cnt[b] else 0
            lift = conf_ab / (item_cnt[b] / n) if item_cnt[b] else 0

            for ant, con, conf in [(a, b, conf_ab), (b, a, conf_ba)]:
                if conf >= min_confidence:
                    # Filter chay/mặn nếu có
                    if dish_type:
                        d_ant = all_dishes.get(ant)
                        tags = (d_ant.tags or "").lower() if d_ant else ""
                        is_veg = "chay" in tags or "chay_duoc" in tags
                        if dish_type == "chay" and not is_veg:
                            continue
                        if dish_type == "man" and is_veg:
                            continue
                    rules.append({
                        "antecedent": names.get(ant, f"#{ant}"),
                        "consequent": names.get(con, f"#{con}"),
                        "support": round(sup, 3),
        ...
        fis = [{"items": [names.get(a, f"#{a}"), names.get(b, f"#{b}")],
                "support": round(c/n, 3), "count": c}
               for (a, b), c in sorted(freq2.items(), key=lambda x: -x[1])[:15]]

        return {"rules": rules[:15], "frequent_itemsets": fis,
                "total_transactions": n, "min_support": min_support, "min_confidence": min_confidence}
